	...
	async function player_ready() {
		...
		check = await check_list(); // check the list to make sure there are no errors
		
		if (check == deck.cards.length) { // called only if check_list checks all cards successfully
	...
	async function add() {
		...
		async function database_search() {
			...
			var blob = await check_in_database();
			
			if (blob != undefined) {
				deck.cards.unshift(add_card_unformatted); // add to top of decklist
				deck.card_imgs.push([add_card_unformatted, blob]) // add to image deck
				console.log(deck.card_imgs);
